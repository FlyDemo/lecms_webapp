$("document").ready(function(){
	listPageData("/lecms_webapp/WorkController/dealteBorrowList");
	
	//绑定同意按钮事件
	$("tr").find("a#agreeBtn").on("click",function(){
		var flowId = $(this).parents("tr.borrowFlowListTr").attr("id");
		$(location).attr("href","/lecms_webapp/WorkController/toDealte?id="+flowId+"&op=agree");
	});
	
	//绑定取消申请按钮
	$("tr").find("a#refuseBtn").on("click",function(){
		var flowId = $(this).parents("tr.borrowFlowListTr").attr("id");
		$(location).attr("href","/lecms_webapp/WorkController/toDealte?id="+flowId+"&op=refuse");
	});
