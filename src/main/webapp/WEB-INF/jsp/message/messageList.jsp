...
<script src="${ctx}/js/message/messageList.js"></script>
        ...
        <td  style="background-color:white;" height="24" bgcolor="#353c44"><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td><table width="100%" cellspacing="0" cellpadding="0">
              <tr>
                <td width="94%" align="center"><span>消息列表</span></td>
              </tr>
            </table></td>
  ...
  <tr>
    <td><table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#a8c7ce" onmouseover="changeto()"  onmouseout="changeback()">
      <tr>
        ...
        <td width="50%" height="20" bgcolor="d3eaef" class="STYLE6"><div align="center"><span class="STYLE10">标题内容</span></div></td>
      ...
      <c:forEach var="messageBean" items="${pageBean.rowDatas}">
	      <tr  id="${messageBean.id}" class="messageListTr">
	      	<input type="hidden" name="isTop" id="isTop" value="${messageBean.topShow}"/>
	      	<td height="20" bgcolor="#FFFFFF" class="STYLE19"><div align="center"><span class="STYLE19">
	      		<c:if test="${messageBean.messageState eq 'NO_READ'}">未读</c:if>
	      		<c:if test="${messageBean.messageState eq 'READED'}">已读</c:if>
	      		<c:if test="${messageBean.topShow eq 'true'}">置顶显示</c:if>
	      	</span></div></td>
	        <td height="20" bgcolor="#FFFFFF" class="STYLE19"><div align="center"><span class="STYLE19">${messageBean.from.name}【
	        		<c:if test="${messageBean.from.leval eq 'ORDINARY'}">普通会员</c:if>
	        		<c:if test="${messageBean.from.leval eq 'REPAIR_MAN'}">维修人员</c:if>
	        		<c:if test="${messageBean.from.leval eq 'SUPER_ADMIN'}">管理员</c:if>
	        					】 发来一个消息...</span></div></td>
	        <td height="20" bgcolor="#FFFFFF" class="STYLE19"<div align="center">${messageBean.createTime}</div></td>
	        <td height="20" bgcolor="#FFFFFF"><div align="center" class="STYLE21"><a id="viewBtn" href="#">查看 |</a>  <a id="isTopBtn" href="#"></a> <a id="deleteBtn" href="#">删除</a></div></td>
	      </tr>
      </c:forEach>
    </table></td>
  </tr>
